...
Light::Light(Vec3d _center, int shadowResX, int shadowResY, float fov) : center(_center), shadowMap(new WorldView("light", 0, 0, shadowResX, shadowResY, fov, -1))
{
...
Vec3d Light::LightPixel(Vec3d point, Vec3d curColor, Vec3d normalVector, Vec3d viewDirection, float kAmbient, Material m) const
{
	...
	return curColor*(kAmbient + (1.0f - kAmbient)*kd + kPhong);
}
